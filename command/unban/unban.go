...
var (
	Command = cli.Command{
		Name:      "unban",
		ShortName: "ub",
		Usage:     "unban -host <hostname> -s <redis|redisproxy|memcache>",
		Action:    unbanAction,
		Flags: []cli.Flag{
			cli.StringFlag{"host", "", "the hostname want to operate"},
			cli.StringFlag{"s", "", "the service want to unban to this host "},
		},
		Description: Usage,
	}
...
func unbanAction(c *cli.Context) {
	h := c.String("host")
	s := c.String("s")
	...
	res := machine.UnBanMachine(h, pos)
	if res == false {
