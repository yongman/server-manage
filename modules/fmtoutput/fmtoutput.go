...
func PrintMachineHeader() {
	fmt.Printf("%5s%30s%8s%7s%5s%5s%5s%15s%15s%5s%5s%5s\n",
		"Type", "Host", "Status", "RM", "L", "RE", "IDC", "Total_Mem(KB)", "Free_Mem(KB)", "B1G", "B5G", "B10G")
}
func PrintMachine(m *db.Machine) {
	fmt.Printf("%5s%30s%8d%7s%5s%5s%5s%15d%15d%5d%5d%5d\n",
		m.Mtype, strings.TrimSuffix(m.Host, ".baidu.com"), m.Status, m.Mroom, m.Logic, m.Region, m.Idc, m.Mem.Total, m.Mem.Free, m.Mem.Box1G, m.Mem.Box5G, m.Mem.Box10G)
...
func PrintAlloc(index int, m *db.Machine, port int32) {
	fmt.Printf("%3d: %30s:%5d\n", index, strings.TrimSuffix(m.Host, ".baidu.com"), port)
}
...
func PrintService(s *db.Service) {
	fmt.Printf("%10s%30s%8d%15d%15d%50s%8s%10s\n",
		s.Stype, strings.TrimSuffix(s.Host, ".baidu.com"), s.Port, s.BoxMem, s.UsedMem, s.DirName, s.Role, s.Version)
}
